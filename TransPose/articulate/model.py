...
class ParametricModel:
    ...
    def view_mesh(self, vertex_list: list, fps=60, distance_between_subjects=0.8, colors: list = None):
        r"""
        ...
        if colors is not None:
            colors = torch.cat(colors, dim = 1).cpu().numpy()
        
        if verts.shape[0] > 1:
            vo3d.render_sequence_3d(verts, faces, 720, 720, 'a.mp4', fps, visible=True, colors = colors)
        else:
            vo3d.vis_mesh(verts[0], faces)

    def view_motion(self, pose_list: list, tran_list: list = None, fps=60, distance_between_subjects=0.8, color_list: list = None):
        r"""
        ...
        :param color_list: List in RGB in range(0, 1), shape is [3]
        """
        verts = []; colors = [] if color_list is not None else None
        for i in range(len(pose_list)):
            ...
            if colors is not None:
                colors.append(torch.Tensor(color_list[i]).view(1, 1, 3).repeat(verts[i].shape[0], verts[i].shape[1], 1))
        self.view_mesh(verts, fps, distance_between_subjects=distance_between_subjects, colors = colors)
