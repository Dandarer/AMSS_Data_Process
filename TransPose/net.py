...
class TransPoseNet(torch.nn.Module):
    ...
    def __init__(self, num_past_frame=20, num_future_frame=5, hip_length=None, upper_leg_length=None,
        ...
        self.pose_s3 = RNN(joint_set.n_full * 3 + n_imu,  joint_set.n_reduced * 6,    128) # 这里估计姿态用6维向量而不是旋转矩阵
        ...
        b = bone_orientation * bone_length # 取消归一化操作，转化为各关节的向量
        b[:3] = 0 # 默认关节为0？， 相对位置
    ...
    def _reduced_glb_6d_to_full_local_mat(self, root_rotation, glb_reduced_pose):
        ...
        :param root_rotation: Tensor in shape [num_frame, x, 3, 3] (IMU measurement of globle rootation)
        :param glb_reduced_pose: Tensor in shape [num_frame, x, n_reduced * 6] (6d represention of net output)
        """
        glb_reduced_pose = art.math.r6d_to_rotation_matrix(glb_reduced_pose).view(-1, joint_set.n_reduced, 3, 3)
        global_full_pose = torch.eye(3, device=glb_reduced_pose.device).repeat(glb_reduced_pose.shape[0], 24, 1, 1) # B, 24, 3, 3
        global_full_pose[:, joint_set.reduced] = glb_reduced_pose
        pose = self.global_to_local_pose(global_full_pose).view(-1, 24, 3, 3) # 忽略的关节默认为000的欧拉角
        pose[:, joint_set.ignored] = torch.eye(3, device=pose.device)
        pose[:, 0] = root_rotation.view(-1, 3, 3) # 腰部为全局姿态
        return pose
    ...
    def forward_offline(self, imu):
        ...
        root_rotation = imu[:, -9:].view(-1, 3, 3) # 这里仅取了腰部IMU数据
    ...
    def forward_online(self, x):
        ...
        # calculate pose (local joint rotation matrices)
        root_rotation = imu[self.num_past_frame, -9:].view(3, 3).cpu() # 这里仅取了腰部IMU数据
